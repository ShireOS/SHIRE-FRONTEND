...
function normalizeWorkspace(value) {
  ...
  return {
    ...
    profiles: {
      server: { ...emptyProfile('server_menu'), ...value?.profiles?.server, browse_department_ids: departmentIds },
...
export default function MenuWorkspaceEditor({ restaurantId, canEdit = true, compact = false }) {
  ...
  return <div className={compact ? 'space-y-4' : 'space-y-5'}>
    ...
    <section className="rounded-lg border border-dash-border bg-[var(--glass-bg)] p-4 sm:p-5">
      ...
      {tab === 'server' && <div className="space-y-5"><label className="block max-w-sm text-sm font-semibold">Default server view<select disabled={!canEdit} value={workspace.profiles.server.default_open?.kind === 'shortcuts' ? 'shortcuts' : 'departments'} onChange={(event) => updateProfile('server', { ...workspace.profiles.server, default_open: event.target.value === 'shortcuts' ? { kind: 'shortcuts' } : null })} className="mt-1 h-10 w-full rounded-md border border-dash-border bg-transparent px-3"><option value="departments">All departments</option><option value="shortcuts">Quick Menu</option></select></label><ShortcutEditor title="Server Quick Menu" description="Add and rank common server items. There is no fixed shortcut count; the POS shows only what fits after departments." profile={workspace.profiles.server} items={workspace.items} disabled={!canEdit} onChange={(profile) => updateProfile('server', profile)} /></div>}
