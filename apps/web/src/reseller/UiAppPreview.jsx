...
function PreviewFrame({
  ...
  useEffect(() => sendState(), [componentOverrides, mode, service, tokens, url])
...
export default function UiAppPreview({
  ...
  return <>
    <div className="overflow-x-auto pb-2"><PreviewFrame service={service} tokens={tokens} componentOverrides={componentOverrides} mode={mode} onComponentSelect={onComponentSelect} /></div>
    ...
    {expanded && <div className="fixed inset-0 z-[80] flex flex-col bg-black/95 p-3 sm:p-5"><div className="mx-auto mb-3 flex w-full max-w-[1440px] items-center justify-between text-white"><div><strong>Real {service === 'pos' ? 'POS' : 'Host'} sandbox</strong><p className="text-xs text-white/60">Uses the service's actual components with in-memory preview data.</p></div><button type="button" title="Close full-screen preview" onClick={() => setExpanded(false)} className="grid h-10 w-10 place-items-center rounded-md border border-white/20"><X size={18} /></button></div><div className="mx-auto min-h-0 w-full max-w-[1440px] flex-1"><PreviewFrame service={service} tokens={tokens} componentOverrides={componentOverrides} mode={mode} onComponentSelect={onComponentSelect} expanded /></div></div>}
  </>
}
