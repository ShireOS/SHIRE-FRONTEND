  ...
  return (
    <div className="space-y-5">
      ...
      <SectionShell title="Scheduled and recurring pricing" description="Expired rows remain here for audit. Pause or archive active rules without changing historical order prices.">
        {batches.length === 0 ? <MenuEmptyState title="No scheduled pricing">Date-window, future, and recurring price rules will appear here.</MenuEmptyState> : <div className="space-y-2">{batches.map(batch => <div key={batch.batch_id} className="flex flex-wrap items-center gap-3 rounded-xl border border-white/10 bg-white/[0.025] p-4"><div className="min-w-0 flex-1"><p className="font-semibold">{batch.name}</p><p className="mt-1 text-sm text-dash-tertiary">{adjustmentLabel(batch)} · {scheduleLabel(batch)} · {batch.count} target{batch.count === 1 ? '' : 's'}</p></div><span className={`rounded-full border px-2.5 py-1 text-xs ${batch.archived_at ? 'border-white/10 text-dash-tertiary' : batch.is_active ? 'border-emerald-400/30 text-emerald-200' : 'border-amber-400/30 text-amber-200'}`}>{batch.archived_at ? 'Archived' : batch.is_active ? 'Enabled' : 'Paused'}</span>{!batch.archived_at && <SmallButton disabled={busy || !canEditPrices} onClick={() => void changeBatch(batch, batch.is_active ? 'pause' : 'resume')}>{batch.is_active ? 'Pause' : 'Resume'}</SmallButton>}{!batch.archived_at && <SmallButton variant="danger" disabled={busy || !canEditPrices} onClick={() => void changeBatch(batch, 'archive')}>Archive</SmallButton>}</div>)}</div>}
      </SectionShell>
