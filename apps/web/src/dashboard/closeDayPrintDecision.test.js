...
test('queued print work is a reviewed decision instead of an absolute readiness blocker', () => {
  ...
  assert.match(page, /Wait and review on POS/)
  assert.match(page, /Discard during Close Day/)
...
test('the warning distinguishes server queue records from physical and POS-local work', () => {
  assert.match(page, /Paper already sent to or printed by a printer cannot be recalled\./)
  assert.match(page, /POS-local held or dead-letter work is not included/)
