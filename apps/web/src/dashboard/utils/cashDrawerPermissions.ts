...
export function cashDrawerRoleSummary(
  ...
  const movementApproval = managerOverride || roleApproval
  const threshold = policy.cash_drop_threshold == null || policy.cash_drop_threshold === ''
    ? null
    : Number(policy.cash_drop_threshold)

  return [
    { key: 'no_sale', label: 'No Sale', value: canNoSale ? (managerOverride ? 'manager PIN' : 'role approved') : 'manager only' },
    { key: 'paid_in', label: 'Paid In', value: canMoveCash ? (movementApproval ? 'manager PIN' : 'role approved') : 'not allowed' },
    { key: 'paid_out', label: 'Paid Out', value: canMoveCash ? 'manager PIN' : 'not allowed' },
    {
      ...
      value: !canMoveCash
        ? 'not allowed'
        : movementApproval || threshold == null || !Number.isFinite(threshold)
          ? 'manager PIN'
