...
function MetricCard({ label, value, detail, comparison, comparisonKind = 'money' }) {
  const delta = comparison?.delta
  const hasPriorData = comparison && Number(comparison.previous) !== 0
  const formattedDelta = comparisonKind === 'count' ? count(Math.abs(delta)) : money(Math.abs(delta))
  return (
    <div className="glass-card min-w-0 rounded-2xl p-5">
      ...
      {hasPriorData && (
        <p className={`mt-1 text-xs font-semibold ${Number(delta) >= 0 ? 'text-dash-success' : 'text-dash-danger'}`}>
          {Number(delta) > 0 ? '+' : Number(delta) < 0 ? '−' : ''}{formattedDelta}
          {comparison.percent != null ? ` (${Number(comparison.percent) > 0 ? '+' : ''}${Number(comparison.percent).toFixed(1)}%)` : ''}
          {' '}from prior period
        </p>
      )}
      {comparison && !hasPriorData && <p className="mt-1 text-xs text-dash-tertiary">No prior-period data</p>}
...
export default function OverviewPage({ restaurantBase = '/restaurants' }) {
  ...
  const saveLayout = async (modules) => {
    ...
    try {
      await fetchWithSupabaseAuth('/portfolio-reports/preferences', {
        method: 'PUT',
        body: JSON.stringify({ visible_modules: modules }),
      })
      ...
      {tab === 'email' ? <PortfolioEmailPanel /> : viewHydrated && (
        <>
          ...
          <div className={`grid gap-4 md:grid-cols-2 ${visible('profit_after_labor') ? 'xl:grid-cols-5' : 'xl:grid-cols-4'}`}>
            ...
            <MetricCard label="Orders" value={count(totals.orders)} comparison={comparisonFor('orders')} comparisonKind="count" />
            <MetricCard label="Covers" value={count(totals.covers)} comparison={comparisonFor('covers')} comparisonKind="count" />
              ...
              <div className="h-72 min-w-0">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={report.trends || []} margin={{ left: 4, right: 18, top: 12, bottom: 4 }}>
                    ...
                    <XAxis dataKey="bucket" tickFormatter={(value) => String(value).slice(5, 10)} axisLine={{ stroke: '#78716c' }} tickLine={{ stroke: '#78716c' }} tick={{ fill: '#a8a29e', fontSize: 11 }} />
                    <YAxis tickFormatter={(value) => money(value)} width={82} axisLine={false} tickLine={false} tick={{ fill: '#a8a29e', fontSize: 11 }} />
                    <Tooltip formatter={(value) => [money(value), 'Net sales']} labelFormatter={(value) => new Date(value).toLocaleString()} cursor={{ stroke: '#78716c', strokeDasharray: '4 4' }} contentStyle={{ background: '#ffffff', border: '1px solid #d6d3d1', borderRadius: 8, color: '#1c1917' }} labelStyle={{ color: '#57534e', fontWeight: 600 }} itemStyle={{ color: '#156cc2', fontWeight: 700 }} />
                    <Line type="monotone" dataKey="net_sales" stroke="#4f7ee8" strokeWidth={3} dot={{ r: 4, fill: '#4f7ee8', stroke: '#ffffff', strokeWidth: 2 }} activeDot={{ r: 6, fill: '#156cc2', stroke: '#ffffff', strokeWidth: 2 }} connectNulls isAnimationActive={false} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </section>
