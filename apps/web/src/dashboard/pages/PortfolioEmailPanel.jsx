          ...
          <article key={recipient.id} className="glass-card rounded-2xl p-4">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center">
              ...
              <button type="button" disabled={!data.can_manage} onClick={() => setEditing(recipient)} className="min-w-0 flex-1 text-left disabled:cursor-default">
                ...
                <p className="mt-1 text-xs text-dash-tertiary">{recipient.frequency} at {formatTimeLabel(recipient.send_time)} · {recipient.scope_mode === 'all' ? 'All groups' : `${recipient.group_ids?.length || 0} groups${recipient.include_ungrouped ? ' + Ungrouped' : ''}`}</p>
              </button>
            ...
            <article key={recipient.id} className="glass-card rounded-2xl p-4">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                ...
                <div className="min-w-0 flex-1">
                  <p className="truncate font-semibold">{recipient.name || recipient.email}</p>
                  <p className="truncate text-sm text-dash-secondary">{recipient.email}</p>
                  <p className="mt-1 text-xs text-dash-tertiary">{recipient.restaurant_name} · {recipient.frequency} at {formatTimeLabel(recipient.send_time)} · {recipient.timezone}</p>
                </div>
