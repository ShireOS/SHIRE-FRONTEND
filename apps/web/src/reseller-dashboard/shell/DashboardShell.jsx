...
function deriveRoleLabel(auth) {
  ...
  if (auth.accountType === 'reseller_employee') return 'Reseller staff'
...
function LocationSwitcher({ context, restaurantId, activeTab }) {
  ...
  const handleChange = async (event) => {
    ...
    try {
      ...
      navigate(`/reseller/restaurants/${nextId}/${context === 'store' ? activeTab || 'analytics' : 'analytics'}`)
    } finally {
      setSwitching(false)
    }
  }
      ...
      {sidebarOpen && (
        ...
        >
          <div className="px-5 pb-1 pt-6">
            <Link to="/reseller" className="font-display text-2xl tracking-tight text-dash-cream">
              SHIRE
            </Link>
          ...
          <div className="flex-1 overflow-y-auto px-2 pb-4">
            ...
            <SidebarItem
              ...
              onClick={() => navigate('/reseller/overview')}
            />
            <SidebarItem
              icon={Building2}
              label="Stores"
            ...
            {showRates && (
              <SidebarItem
                ...
                onClick={() => navigate('/reseller/rates')}
              />
            )}
            <SidebarItem
              icon={Monitor}
                ...
                {storeNav.map((item) => (
                  item.children ? (
                    <SidebarGroup
                      ...
                      onNavigate={(childId, sectionId) => navigate(`/reseller/restaurants/${restaurantId}/${childId}${sectionId ? `#${sectionId}` : ''}`)}
                    />
                  ) : (
                    <SidebarItem
                      ...
                      onClick={() => navigate(`/reseller/restaurants/${restaurantId}/${item.id}${item.section ? `#${item.section}` : ''}`)}
                    />
                  )
                ))}
