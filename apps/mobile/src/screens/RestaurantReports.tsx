...
function EmailModal({ visible, recipients, canManage, onClose, onSave, onDelete }: { visible: boolean; recipients: ReportRecipient[]; canManage: boolean; onClose: () => void; onSave: (value: Omit<ReportRecipient, 'id'>, id?: string) => Promise<void>; onDelete: (id: string) => Promise<void> }) {
  const [draft, setDraft] = useState<Omit<ReportRecipient, 'id'> | null>(null);
  const [editingId, setEditingId] = useState<string | undefined>();
  const [saving, setSaving] = useState(false);
  const edit = (recipient?: ReportRecipient) => { setEditingId(recipient?.id); setDraft(recipient ? { ...recipient } : { ...EMPTY_RECIPIENT }); };
