...
function DailySection({ section, statWidth, comparisonEnabled }: { section: any; statWidth: any; comparisonEnabled: boolean }) {
  ...
  const comparison = comparisonEnabled ? section.comparison || {} : {};
  const cash = section.cash_accountability || {};
  return <ReportSection title={`Daily summary · ${section.business_date || ''}`} subtitle="Profit after labor excludes food and other operating costs."><View style={styles.statGrid}><Stat width={statWidth} label="Net sales" value={formatMoney(summary.net_revenue)} comparison={comparison.net_revenue} comparisonFormat={formatMoney} /><Stat width={statWidth} label="Employee cost" value={formatMoney(summary.labor_cost)} comparison={comparison.labor_cost} comparisonFormat={formatMoney} invertDelta /><Stat width={statWidth} label="Profit after labor" value={formatMoney(summary.labor_adjusted_profit)} comparison={comparison.labor_adjusted_profit} comparisonFormat={formatMoney} /><Stat width={statWidth} label="Tickets" value={formatNumber(summary.ticket_count)} comparison={comparison.ticket_count} /><Stat width={statWidth} label="Expected cash" value={cash.variance_available ? formatMoney(cash.expected_cash) : 'Not closed'} /><Stat width={statWidth} label="Counted cash" value={cash.variance_available ? formatMoney(cash.counted_cash) : 'Not closed'} /><Stat width={statWidth} label="Cash variance" value={cash.variance_available ? formatMoney(cash.variance) : 'Not closed'} /></View>{(section.flags || []).map((flag: any, index: number) => <View key={`${flag.message}-${index}`} style={styles.flag}><Text style={styles.flagText}>{flag.message}</Text></View>)}<Disclosure title="Cash movements" count={(section.cash_movements || []).length}><DataTable columns={[['movement_type', 'Type'], ['amount', 'Amount'], ['performed_by_name', 'Performed'], ['approved_by_name', 'Approved'], ['status', 'Status']]} rows={section.cash_movements || []} formatters={{ amount: formatMoney }} /></Disclosure><DataTable columns={[['name', 'Top item'], ['units', 'Units'], ['revenue', 'Revenue'], ['margin', 'Margin']]} rows={section.top_items || []} formatters={{ revenue: formatMoney, margin: formatMoney }} /></ReportSection>;
}
